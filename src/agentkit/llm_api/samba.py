...
def initialize_client():
    ...
    if os.environ.get("SAMBANOVA_KEY"):
        return OpenAI(
            api_key=os.environ["SAMBANOVA_KEY"],
            base_url="https://api.sambanova.ai/v1",
        )
    openai_key_path = os.path.join(os.path.expanduser("~"), ".openai", "sambanova.key")
    ...
    if os.path.exists(openai_key_path):
        ...
        raise FileNotFoundError(
            """
            Please create a file at ~/.openai/sambanova.key with your Sambanova API key. 
